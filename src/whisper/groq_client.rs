...
impl GroqClient {
    pub fn new(api_key: String, endpoint: Option<String>) -> Result<Self> {
        ...
        info!("Initialized Groq client with endpoint: {}", endpoint);
    ...
    pub async fn transcribe(&self, audio_path: &PathBuf, model: &str, language: &str) -> Result<String> {
        ...
        // Set temperature to 0 for consistent transcription
        form = form.text("temperature", "0");
        
        debug!("Sending request to Groq API with model: {}, language: {}", model, language);
        ...
        if !status.is_success() {
            error!("Groq API request failed with status {}: {}", status, response_text);
